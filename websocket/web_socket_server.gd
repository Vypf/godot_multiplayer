...
func _connect_pending(p: PendingPeer) -> bool:
	if p.ws != null:
		return _handle_websocket_connection(p)
	...
	return _handle_tls_connection(p)


func _handle_websocket_connection(p: PendingPeer) -> bool:
...
func _handle_tls_connection(p: PendingPeer) -> bool:
